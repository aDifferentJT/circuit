...
greedySelectImplicants' uncovered implicants = do
  (implicant, covered) <- the (ExceptT String RandomM (List (Literal a), List (List (Literal a))))
                        $ exceptFromMaybe "Could not select implicants"
                        $ rndSelect'
                        $ fst
                        $ the (List (List (Literal a), List (List (Literal a))), Nat)
                        $ foldl
