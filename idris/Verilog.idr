...
putMessage : String -> MonadState (List String) m => m ()
putMessage e = modify f
  where
      ...
      map (unlines . concat) $ sequence $ the (List (StateT (List String) RandomM (List String)))
        [ concat <$> traverse
          (\(primName, (primInput ** primOutput ** (f, _, _))) => the (StateT (List String) RandomM (List String)) $ do
            ...
            case res of
                 Left e => do
                   putMessage $ "Failed to analyse primitive: " ++ primName
                   putMessage e
                   pure []
                 Right outputs => pure $ concat $ the (List (List String))
                   [ [ "module " ++ primName ]
                   , concat $ Functor.map (map ("  " ++)) $ the (List (List String))
                     [ mapNilHeadTail ["("] ("( " ++) (", " ++)
        ...
        , pure $ concat $ Functor.map (map ("  " ++)) $ the (List (List String))
          [ mapNilHeadTail ["("] ("( " ++) (", " ++)
          ...
          , concatMap
            (\((primName, primHash), (_ ** _ ** (_, x, y))) => concat $ the (List (List String))
              ...
              , concatMap (map ("  " ++)) $ the (List (List String))
                [ mapNilHeadTail ["("] ("( " ++) (", " ++)
